...
The ability to run tasks in parallel has been supported for quite some time, however only inside of a stage. The pipeline visualization will still only show one stage and not several. I suppose BlueOcean fixes has a better visualization of this but it also has some drawbacks (the amount of plugins required to install BlueOcean is staggering).
